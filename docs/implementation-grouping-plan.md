...
- [Dependency graph](#dependency-graph)
  - [Mermaid diagram](#dependency-graph-mermaid-diagram)
...
<a id="dependency-graph-mermaid-diagram"></a>
### Mermaid diagram

```mermaid
graph TD
  G1[1. Foundational contract + shared runtime utilities]
  G2[2. (A) SpiceCell/SpiceWindow types + basic ops]
  G3[3. (B) String array + output-buffer conventions]
  G4[4. (D) Error system + status utilities]
  G5[5. Kernel pool read/write core]
  G6[6. IDs↔names & frame name utilities]
  G7[7. Time expansions (non-core conversions)]
  G8[8. Vector/matrix bulk add (no kernels required)]
  G9[9. Frame transforms & attitude (CK read-only first)]
  G10[10. (C) File handles + DAF/DAS/DLA core]
  G11[11. Kernel management “full”]
  G12[12. SPK read APIs + coverage/object queries]
  G13[13. Geometry “classic”]
  G14[14. GF event finding tranche 1]
  G15[15. Writers: SPK/CK/PCK]
  G16[16. DSK + EK “big subsystems”]
