...
export const nativeReturnBindings: readonly NativeReturnBindingEntry[] = [
  {
    id: "cells-windows.insrtc",
    enumId: "V2_FUNCTION_ID_CELLS_WINDOWS_INSRTC",
    cSymbol: "insrtc_c",
    kind: "generatedReturnBindingLane",
  },
  {
...
const nativeReturnBindingById = new Map<string, NativeReturnBindingEntry>();
for (const entry of nativeReturnBindings) {
  nativeReturnBindingById.set(entry.id, entry);
}

export function lookupNativeReturnBindingEntry(fnId: string): NativeReturnBindingEntry | undefined {
  return nativeReturnBindingById.get(fnId);
}
