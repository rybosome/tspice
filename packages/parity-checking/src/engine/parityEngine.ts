...
const CANONICAL_REFERENCE_LANE = "cspice-native" as const;
const CANONICAL_COMPARISON_LANES = ["node", "wasm"] as const;
...
export type ParityEngineSummary = {
  skipped: boolean;
  skipReason?: string;
  referenceLane: typeof CANONICAL_REFERENCE_LANE;
  comparisonLanes: readonly [...typeof CANONICAL_COMPARISON_LANES];
...
export async function runParityEngine(): Promise<ParityEngineSummary> {
  ...
  try {
    paritySummary = await withProofRunners(async (runners) => {
      ...
      return {
        skipped: false,
        referenceLane: CANONICAL_REFERENCE_LANE,
        comparisonLanes: [...CANONICAL_COMPARISON_LANES],
        fallbackUsed: false,
        workflowCount: 0,
