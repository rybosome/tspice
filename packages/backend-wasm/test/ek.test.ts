...
describe("@rybosome/tspice-backend-wasm ek", () => {
  it("can query table metadata and open/close EK handles", async () => {
  ...
  it("supports ekfind + ekg* and fast-write roundtrip", async () => {
    const backend = await createWasmBackend();
    backend.kclear();

    const peopleEkPath = "generated/people.bes";
    ...
    // Load the EK we just wrote and query it.
    ...
    // Vector-valued entry: verify `found: false` for out-of-range element.
    const vecEkPath = "generated/vector.bes";
    ...
    // Query parse failures should be surfaced via return value (not throw).
