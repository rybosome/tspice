...
describe("SPK writers (type 8)", () => {
  ...
  itNative("writes a minimal type 8 segment and reads it back", () => {
    ...
    backend.raw.furnsh({ path: output.path, bytes });

    const { state } = backend.raw.spkezr("1000", 30, "J2000", "NONE", "0");
  ...
  itNative("rejects path traversal in VirtualOutput.path", () => {
    ...
    expect(() =>
      backend.raw.spkopn({ kind: "virtual-output", path: "../evil.bsp" }, "TSPICE", 0),
    ).toThrow(/\.\.|invalid/i);
  });
