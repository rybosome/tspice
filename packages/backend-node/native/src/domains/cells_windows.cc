...
bool ReadHandleArg(
  ...
  if (!value.IsNumber()) {
    ThrowSpiceError(
        Napi::TypeError::New(env, std::string(label) + ": expected a numeric handle"));
    return false;
  }
  const uint32_t h = value.As<Napi::Number>().Uint32Value();
  if (h == 0) {
    ThrowSpiceError(
        Napi::TypeError::New(env, std::string(label) + ": handle must be a non-zero integer"));
    return false;
  }
  if (outHandle) {
    *outHandle = h;
...
uintptr_t GetCellPtrOrThrow(Napi::Env env, uint32_t handle, const char* context) {
  uintptr_t ptr = 0;
  if (!TryGetCellPtr(handle, &ptr) || ptr == 0) {
    ThrowSpiceError(
        env,
        std::string(context) + ": unknown/expired cell handle: " + std::to_string(handle));
    return 0;
  }
  return ptr;
}
...
static Napi::Number NewIntCell(const Napi::CallbackInfo& info) {
  ...
  const uint32_t handle = AddCellHandle(ptr);
...
static void FreeHandleCommon(Napi::Env env, uint32_t handle, const char* context) {
  std::lock_guard<std::mutex> lock(tspice_backend_node::g_cspice_mutex);

  uintptr_t ptr = 0;
  if (!RemoveCellPtr(handle, &ptr) || ptr == 0) {
    ThrowSpiceError(env, std::string(context) + ": unknown/expired cell handle");
    return;
  }

  char err[tspice_backend_node::kErrMaxBytes];
  const int code = tspice_free_cell(ptr, err, (int)sizeof(err));
  if (code != 0) {
...
static Napi::Value FreeCell(const Napi::CallbackInfo& info) {
  ...
  uint32_t handle = 0;
  if (!ReadHandleArg(env, info[0], "cell", &handle)) {
    return env.Undefined();
  }

  FreeHandleCommon(env, handle, "freeCell");
  return env.Undefined();
}
...
static Napi::Value FreeWindow(const Napi::CallbackInfo& info) {
  ...
  uint32_t handle = 0;
  if (!ReadHandleArg(env, info[0], "window", &handle)) {
    return env.Undefined();
  }

  FreeHandleCommon(env, handle, "freeWindow");
  return env.Undefined();
}
...
static Napi::Value Ssize(const Napi::CallbackInfo& info) {
  ...
  const int size = info[0].As<Napi::Number>().Int32Value();
  uint32_t handle = 0;
  if (!ReadHandleArg(env, info[1], "cell", &handle)) {
    return env.Undefined();
  }

  std::lock_guard<std::mutex> lock(tspice_backend_node::g_cspice_mutex);
  const uintptr_t ptr = GetCellPtrOrThrow(env, handle, "ssize");
...
static Napi::Value Scard(const Napi::CallbackInfo& info) {
  ...
  std::lock_guard<std::mutex> lock(tspice_backend_node::g_cspice_mutex);
  const uintptr_t ptr = GetCellPtrOrThrow(env, handle, "scard");
  if (env.IsExceptionPending()) return env.Undefined();
...
static Napi::Number Card(const Napi::CallbackInfo& info) {
  ...
  std::lock_guard<std::mutex> lock(tspice_backend_node::g_cspice_mutex);
  const uintptr_t ptr = GetCellPtrOrThrow(env, handle, "card");
...
static Napi::Number Size(const Napi::CallbackInfo& info) {
  ...
  std::lock_guard<std::mutex> lock(tspice_backend_node::g_cspice_mutex);
  const uintptr_t ptr = GetCellPtrOrThrow(env, handle, "size");
  if (env.IsExceptionPending()) return Napi::Number::New(env, 0);
...
static Napi::Value Valid(const Napi::CallbackInfo& info) {
  ...
  uint32_t handle = 0;
  if (!ReadHandleArg(env, info[2], "cell", &handle)) {
    return env.Undefined();
  }

  std::lock_guard<std::mutex> lock(tspice_backend_node::g_cspice_mutex);
  const uintptr_t ptr = GetCellPtrOrThrow(env, handle, "valid");
  if (env.IsExceptionPending()) return env.Undefined();
...
static Napi::Value Insrti(const Napi::CallbackInfo& info) {
  ...
  std::lock_guard<std::mutex> lock(tspice_backend_node::g_cspice_mutex);
  const uintptr_t ptr = GetCellPtrOrThrow(env, handle, "insrti");
  if (env.IsExceptionPending()) return env.Undefined();
...
static Napi::Value Insrtd(const Napi::CallbackInfo& info) {
  ...
  std::lock_guard<std::mutex> lock(tspice_backend_node::g_cspice_mutex);
  const uintptr_t ptr = GetCellPtrOrThrow(env, handle, "insrtd");
  if (env.IsExceptionPending()) return env.Undefined();
...
static Napi::Value Insrtc(const Napi::CallbackInfo& info) {
  ...
  std::lock_guard<std::mutex> lock(tspice_backend_node::g_cspice_mutex);
  const uintptr_t ptr = GetCellPtrOrThrow(env, handle, "insrtc");
  if (env.IsExceptionPending()) return env.Undefined();
...
static Napi::Number CellGeti(const Napi::CallbackInfo& info) {
  ...
  std::lock_guard<std::mutex> lock(tspice_backend_node::g_cspice_mutex);
  const uintptr_t ptr = GetCellPtrOrThrow(env, handle, "cellGeti");
  if (env.IsExceptionPending()) return Napi::Number::New(env, 0);
...
static Napi::Number CellGetd(const Napi::CallbackInfo& info) {
  ...
  std::lock_guard<std::mutex> lock(tspice_backend_node::g_cspice_mutex);
  const uintptr_t ptr = GetCellPtrOrThrow(env, handle, "cellGetd");
  if (env.IsExceptionPending()) return Napi::Number::New(env, 0);
...
static Napi::Value Wninsd(const Napi::CallbackInfo& info) {
  ...
  uint32_t handle = 0;
  if (!ReadHandleArg(env, info[2], "window", &handle)) {
    return env.Undefined();
  }

  std::lock_guard<std::mutex> lock(tspice_backend_node::g_cspice_mutex);
  const uintptr_t ptr = GetCellPtrOrThrow(env, handle, "wninsd");
  if (env.IsExceptionPending()) return env.Undefined();
...
static Napi::Number Wncard(const Napi::CallbackInfo& info) {
  ...
  std::lock_guard<std::mutex> lock(tspice_backend_node::g_cspice_mutex);
  const uintptr_t ptr = GetCellPtrOrThrow(env, handle, "wncard");
  if (env.IsExceptionPending()) return Napi::Number::New(env, 0);
...
static Napi::Array Wnfetd(const Napi::CallbackInfo& info) {
  ...
  std::lock_guard<std::mutex> lock(tspice_backend_node::g_cspice_mutex);
  const uintptr_t ptr = GetCellPtrOrThrow(env, handle, "wnfetd");
...
static Napi::Value Wnvald(const Napi::CallbackInfo& info) {
  ...
  std::lock_guard<std::mutex> lock(tspice_backend_node::g_cspice_mutex);
  const uintptr_t ptr = GetCellPtrOrThrow(env, handle, "wnvald");
  if (env.IsExceptionPending()) return env.Undefined();
