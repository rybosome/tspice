...
| \`kernels.custom(...)\` | Mission/app-specific kernel sets | Two modes: \`kernels.custom()\` accepts explicit \`{ url, path? }\` entries; \`kernels.custom({ origin, pathBase, baseUrl? })\` also accepts string ids mapped to \`{ url: origin + id, path: pathBase + id }\`. | You own hosting + path conventions. Ensure ordering and coverage match your use case. |
`;
